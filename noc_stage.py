##########Try to optimize latency of mesh###################
#################################### Import ###############################
...
def create_nodes(num_nodes):
    ...
    # nodes[0] = ddr          ## node 0 is ddr(0)

    # for i in range(1, 8):
    #     nodes[i] = cpu      ## node 1-7 are cpu(10-16)
    #     cpu = cpu + 1
    
    # for i in range(8, 16):
    #     nodes[i] = pe       ## node 8-15 are pe(20-27)
    #     pe = pe + 1
...
def construct_architecture(architecture_file):
    with open("/content/drive/My Drive/Colaboratory/NoC_STAGE/"+ architecture_file+".arch") as f:
        content = f.readlines()
        content = [x.strip() for x in content]
        num_nodes = int(content[0][0])      ## line 0 is number of nodes
        arch = []
...
def dijkstra(edges, f, t):      ## edges, node, node
    ...
    q, seen = [(0,f,())], set()
    while q:
        (cost,v1,path) = heappop(q)         ## pop minimal
        if v1 not in seen:
            seen.add(v1)
            path = (v1, path)
    ...
    if (r1 < threshold):  # exchange cores 0.6
        ...
        while 1:
            ...
            for i in range(0, 64):
                ...
                if (cost_trial1>100 or cost_trial2>100):        ## cost is to high, resulting an isolated island, proving this change is not good, return changes back
                    island=1
                    break
...
def main(start_time):
    ...
    #best_phv=0
    global_archive=[]
    local_archive=[] #complete information of NoC    local_archive contains 0:mean  1:deviation  2:nodes[]  3:links[][]  4:time elapsed
    ...
    traffic_file = "robot_mesh_4x4"     ## here we assign traffic_file
    arch_traffic_file = "arch_robot_mesh_4x4"
    re_traffic_file = "reconfig_robot_mesh_4x4"
    architecture_file = "ring_8nodes_16cores"
    ...
    mesh_mean, mesh_dev = calc_params(nodes,links,row,col,traffic)
    ...
    while 1:
        ...
        for a in range(0, num_iter):  # num_iter seperate perturbations from same starting point
            ...
            current_sw_mean, current_sw_dev = calc_params(nodes, links, row, col, traffic)
            ...
            if current_sw_mean < best_mean:
                best_move=deepcopy(last_move)
                best_mean = current_sw_mean
                #best_phv=phv
            else:
        ...
        if(num_try == 2000):
            print("best mean in the process is: ", best_record[0], "corresponding nodes is: ", best_record[1])      ## Result
            reconfigure_traffic(best_record, arch_traffic_file, re_traffic_file)
            break

        if(count1==num_iter): #no improvement in num_iter perturbations, assume reached minima
            num_try += 1
            print("best_mean = ", best_mean, "num_try = ", num_try)
            print('********************************')
            temp_nodes, temp_links = best_perturb(nodes, links, best_move)
            if(best_mean < best_record[0]):
                best_record[0] = best_mean
            ...
            while 1:
                ...
                tm,td=calc_params(nodes,links,row,col,traffic)
                #tm=(1-tm/mesh_mean)*w1
                #td=(1-td/mesh_dev)*w2
                inpt=[]               ## because our label is mean, we can not use it as train set anymore
                ...
                predicted_end_mean=regr.predict([inpt])+bias #adding slight bias         ## Predict phv based on regression tree and new core map
                #print('predicted= ',predicted_end_mean, ' & mmean= ',mmean)
                if(predicted_end_mean<=mmean): # choose this as start point
            ...
            current_sw_mean, current_sw_dev = calc_params(nodes, links, row, col, traffic)
            print("a good solution's mean is: ", current_sw_mean, "nodes are: ", nodes)
