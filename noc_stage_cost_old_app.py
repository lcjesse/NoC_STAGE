...
def make_traffic(arch, traffic_file, arch_traffic_file):
    ...
    with open("/content/drive/My Drive/Colaboratory/NoC_STAGE/"+arch_traffic_file+".stp") as f:
        ...
        num_task = int(temp[2])
        num_edge = int(temp[3])
        print(num_node,num_task,num_edge)
        traffic = [[0 for col in range(num_node)] for row in range(num_node)]
        ## every row refers to src_node; every column refers to dst_node
        for i in range(16+num_task, 16+num_task+num_edge):
            temp1 = content[i].split()
    ...
    ## write traffic into txt file
    t=''
    with open ('/content/drive/My Drive/Colaboratory/NoC_STAGE/input_traffic_cost.txt','w') as q:
        for i in range(len(traffic)):
            for e in range(len(traffic[i])):
                t=t+str(traffic[i][e])+' '
            q.write(t.strip(' '))
...
################################### Load traffic ####################################
def load_traffic():
    with open("/content/drive/My Drive/Colaboratory/NoC_STAGE/input_traffic_cost.txt") as f:
        content = f.readlines()
...
def main(start_time):
    ...
    # h263e h264dl h264dh robot sparse satell sample fpppp
    app_name = "h263e"
    traffic_file = app_name + "_mesh_4x4"     ## here we assign traffic_file
    arch_traffic_file = "arch_" + app_name + "_mesh_4x4"
    re_traffic_file = "reconfig_" + app_name + "_mesh_4x4_cost"
    architecture_file = "mesh_16nodes"
    re_architecture_file = "reconfig_" + app_name + "_mesh_16nodes_cost"
